...
public:
	...
	Camara(D3DXVECTOR3 eye, D3DXVECTOR3 target, D3DXVECTOR3 up, int Ancho, int Alto)
	{
		...
		//Posicion de la cámara en tercera persona
		posCam3P = eye;
		posCam3P.z += 15;
		posCam3P.y += 1;
		...
		//la de proyeccion
		D3DXMatrixPerspectiveFovLH( &proyeccion, D3DX_PI/2.0f, ancho / alto, 0.01f, 1000.0f );
	...
	D3DXMATRIX UpdateCam(float vel, float arriaba, float izqder)
	{
		...
		posCam += refFront * vel/14.0; //valor original es de 10 
		posCam3P += refFront * vel / 14.0; //valor original es de 10 
